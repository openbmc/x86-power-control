    ...
    for (nlohmann::json& gpioConfig : gpios)
    {
        ...
        if (tempGpioData->type == ConfigType::GPIO)
        {
            ...
            if (gpioConfig.contains("Polarity"))
            {
                ...
                {
                    std::string errMsg = "Polarity defined but not properly setup. Please <integer> value only. Currently set to " + polarity;
                    phosphor::logging::log<phosphor::logging::level::ERR>(
                        errMsg.c_str());
                    return -1;
                }
...
int main(int argc, char* argv[])
{
    ...
    {
        if (!setGPIOOutput(resetOutConfig.lineName, !resetOutConfig.polarity, line))
        {
            return -1;
        }
    }
