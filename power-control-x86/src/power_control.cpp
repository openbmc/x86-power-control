...
static bool assertGPIO(const ConfigData& config, gpiod::line& gpioLine)
{
    return setGPIOOutput(config.lineName, config.polarity, gpioLine);
}

static bool deassertGPIO(const ConfigData& config, gpiod::line& gpioLine)
{
    return setGPIOOutput(config.lineName, !config.polarity, gpioLine);
}
...
int main(int argc, char* argv[])
{
    ...
    {
        deassertGPIO(nmiOutConfig, nmiOutLine);
    }
    ...
    {
        if (!deassertGPIO(powerOutConfig, line))
        {
            return -1;
        }
    }
    ...
    {
        if (!deassertGPIO(resetOutConfig, line))
        {
            return -1;
        }
    }
        ...
        powerButtonIface->register_property(
            "ButtonMasked", false, [](const bool requested, bool& current) {
                if (requested)
                {
                    ...
                    if (!deassertGPIO(powerOutConfig, powerButtonMask))
                    {
        ...
        resetButtonIface->register_property(
            "ButtonMasked", false, [](const bool requested, bool& current) {
                if (requested)
                {
                    ...
                    if (!deassertGPIO(resetOutConfig, resetButtonMask))
                    {
                        throw std::runtime_error("Failed to request GPIO");
                        return 0;
                    }
