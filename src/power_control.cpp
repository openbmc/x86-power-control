...
{
    sd_journal_send("MESSAGE=PowerControl: power button pressed long",
                    "PRIORITY=%i", LOG_INFO, "REDFISH_MESSAGE_ID=%s",
                    "OpenBMC.0.1.PowerButtonPressed", NULL);
}
...
static void powerButtonHandler(bool state)
{
    ...
    gpiod::line gpioLine;
    bool output_state = asserted ? powerOutConfig.polarity
    ...
    if (asserted)
    {
        powerButtonLongPressTimer.expires_after(
            std::chrono::milliseconds(longPressTimeMs));
        powerButtonLongPressTimer.async_wait(
            [](const boost::system::error_code ec) {
            if (ec)
            {
                // operation_aborted is expected if timer is canceled before
                ...
                {
                    lg2::error(
                        "Power button long press timer failed: {ERROR_MSG}",
                        "ERROR_MSG", ec.message());
                }
            }
            else
...
static void resetButtonHandler(bool state)
{
    ...
    gpiod::line gpioLine;
    bool output_state = asserted ? resetOutConfig.polarity
