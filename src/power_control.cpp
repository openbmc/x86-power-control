...
PersistentState::PersistentState()
{
    ...
    try
    {
        ...
        {
            lg2::info("Cannot parce state file \'{PATH}\'", "PATH",
                      std::string(powerControlDir / stateFile));
            stateData = nlohmann::json({});
            return;
        }
...
void PowerRestoreController::run()
{
    ...
    if (matches.empty())
    {
        ...
        matches.emplace_back(
            *conn,
            match_rules::propertiesChanged(powerRestorePolicyObject,
...
void PowerRestoreController::setProperties(const dbusPropertiesList& props)
{
    for (auto& [property, propValue] : props)
    {
        ...
        {
            const uint16_t* value = std::get_if<uint16_t>(&propValue);
            if (value == nullptr)
            {
                lg2::error("Unable to read Power Restore Delay");
                continue;
            }
            powerRestoreDelay = *value;
        }
#ifdef USE_ACBOOT
...
void PowerRestoreController::invokeIfReady()
{
    ...
    matches.clear();
    if ((powerRestoreDelay > 0) && (timerState != TimerState::Finished))
    {
